...
rversion(Fcall *rx, Fcall *tx)
{
	if(msize > rx->msize)
		msize = rx->msize;
	tx->msize = msize;
	if(rx->version != nil && strncmp(rx->version, "9P", 2) != 0)
		tx->version = "unknown";
	else
		tx->version = "9P2000";
}
...
adduser(struct passwd *p)
{
	...
	u = emalloc(sizeof(*u));
	u->id = 1000; // p->pw_uid;
	u->name = estrdup("mvshaney"); // p->pw_name);
	u->next = utab[/*p->pw_uid*/ 1000%nelem(utab)];
	u->defaultgid = 1000; //p->pw_gid;
	utab[1000/*p->pw_uid*/%nelem(utab)] = u;
	return u;
}
...
addgroup(struct group *g)
{
	...
	for(p=g->gr_mem; *p; p++)
		u->mem[n++] = estrdup(*p);
	u->mem[n++] = estrdup("mvshaney");
	u->nmem = n;
	u->id = 1000; // g->gr_gid;
	u->name = estrdup("mvshaney"); //g->gr_name);
	u->next = gtab[1000/*g->gr_gid*/%nelem(gtab)];
	gtab[1000/*g->gr_gid*/%nelem(gtab)] = u;
	return u;
}
...
uname2user(char *name)
{
	...
	for(i=0; i<nelem(utab); i++)
		for(u=utab[i]; u; u=u->next)
			if(strcmp(u->name, "mvshaney"/*name*/) == 0)
				return u;
...
uid2user(int id)
{
	...
	for(u=utab[/*id*/1000%nelem(utab)]; u; u=u->next)
		if(u->id == id)
			return u;
